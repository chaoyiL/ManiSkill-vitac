...
from policy.model.common.dict_of_tensor_mixin import DictOfTensorMixin
        ...
        if isinstance(x, dict):
            ...
            for key, value in x.items():
                if key in self.params_dict:
                    ...
                    # Pass through unchanged if no normalization parameters (e.g., images)
                    result[key] = value
            return result
        else:
...
data = {
    'obs': torch.zeros((1000,128,9,2)),
    'action': torch.zeros((1000,128,2))
}
normalizer = LinearNormalizer()
normalizer.fit(data)
normalized_data = normalizer.normalize(data)  # Returns normalized dictionary


# Example with single tensor
data = torch.zeros((100,10,9,2))
normalizer = SingleFieldLinearNormalizer()
normalizer.fit(data)
normalized_data = normalizer.normalize(data)  # Returns normalized tensor
