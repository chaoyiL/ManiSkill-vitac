...
from policy.model.tactile.pointnet import PointNetFeatureExtractor
from policy.model.common.low_dim_encoder import create_low_dim_encoder
    ...
    def __init__(self,
        ...
        if rgb_keys:
            ...
            for rgb_key in rgb_keys:
                ...
                print(f"Created ViT encoder for {rgb_key}, time:{time.time() - tic:.2f}s")
        ...
        if tactile_img_keys:
            if share_tactile_model:
                ...
                print(f"Created shared {'ResNet' if use_resnet_for_tactile else 'ViT'} encoder for tactile images, time:{time.time() - tic:.2f}s")
                ...
                if use_resnet_for_tactile:
                    ...
                    print(f"Created shared MlpHead for tactile: {backbone_dim} -> {tactile_mlp_output_dim}, time:{time.time() - tic:.2f}s")
            else:
                ...
                for tactile_key in tactile_img_keys:
                    ...
                    print(f"Created {'ResNet' if use_resnet_for_tactile else 'ViT'} encoder for {tactile_key}, time:{time.time() - tic:.2f}s")
                    ...
                    if use_resnet_for_tactile:
                        ...
                        print(f"Created MlpHead for {tactile_key}: {backbone_dim} -> {tactile_mlp_output_dim} , time:{time.time() - tic:.2f}s")
        ...
        if tactile_pc_keys:
            if share_pointnet_model:
                ...
                print(f"Created shared PointNet encoder for tactile point clouds, time:{time.time() - tic:.2f}s")
                ...
                for pc_key in tactile_pc_keys:
                    ...
                    print(f"Created PointNet encoder for {pc_key}, time:{time.time() - tic:.2f}s")
                print(f"  Augmentation: rotation={pointnet_aug_rotation}, jitter={pointnet_aug_jitter}, scaling={pointnet_aug_scaling}, dropout={pointnet_aug_dropout}")
        ...
        if self.low_dim_keys:
            if self.low_dim_encoder is not None:
                ...
                for key in self.low_dim_keys:
                    ...
                    B, T = data.shape[:2]
                    assert B == batch_size
                    assert data.shape[2:] == self.key_shape_map[key], f"Key: {key}, Data shape: {data.shape}, Key shape map: {self.key_shape_map[key]}"
